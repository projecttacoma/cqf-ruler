...
import org.opencds.cqf.dstu3.utils.LibraryResourceUtils;
...
public class LibraryProvider extends LibraryResourceProvider {

    private NarrativeProvider narrativeProvider;

    public LibraryProvider(NarrativeProvider narrativeProvider) {
    ...
    private CqlTranslator getTranslator(Library library) {
        ...
        CqlTranslator translator = CqlTranslationUtils.getTranslator(
    ...
    private void formatCql(Library library) {
        for (Attachment att : library.getContent()) {
            if (att.getContentType().equals("text/cql")) {
                try {
                    CqlFormatterVisitor.FormatResult fr = CqlFormatterVisitor.getFormattedOutput(
            ...
            for (org.hl7.elm.r1.IncludeDef def : elm.getIncludes().getDef()) {
                library.addRelatedArtifact(
                        new RelatedArtifact()
                                ...
                                .setResource(new Reference().setReference(LibraryResourceUtils.resolveLibrary(this, def.getPath(), def.getVersion()).getId()))
                );
            }
        }
    }
