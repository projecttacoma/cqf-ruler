...
public class BundleProvider extends BundleResourceProvider
{
    private JpaDataProvider provider;

    public BundleProvider(JpaDataProvider provider)
    {
        this.provider = provider;
    }

    @Operation(name = "$apply-cql", idempotent = true)
    public Bundle apply(@IdParam IdType id)
    ...
    @Operation(name = "$apply-cql", idempotent = true)
    public Bundle apply(@ResourceParam Bundle bundle)
            throws FHIRException
    {
        return applyCql(bundle);
    }
    ...
    @Operation(name = "$apply-cql", idempotent = true)
    public Resource apply(@ResourceParam Resource resource)
            throws FHIRException
    {
        return applyCql(resource);
    }

    public Bundle applyCql(Bundle bundle)
            throws FHIRException
    {
    ...
    public Resource applyCql(Resource resource)
            throws FHIRException
    {
        Library library;
        Context context;
            ...
            for (Base base : child.getValues())
            {
                if (base != null)
                {
                    ...
                    if (!extension.isEmpty())
                    {
                        ...
                        library = CqlTranslationUtils.translateLibrary(cql, new LibraryManager(new ModelManager()), new ModelManager());
