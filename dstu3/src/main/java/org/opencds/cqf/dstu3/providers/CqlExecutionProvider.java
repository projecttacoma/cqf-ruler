...
import org.opencds.cqf.dstu3.utils.Dstu3LibrarySourceProvider;
import org.opencds.cqf.dstu3.utils.MeasureBundler;
...
public class CqlExecutionProvider
{
    ...
    private Dstu3LibraryLoader getLibraryLoader()
    {
        if (libraryLoader == null)
        {
            libraryLoader = new Dstu3LibraryLoader((LibraryDao) provider.resolveResourceProvider("Library").getDao());
        }
        return libraryLoader;
    }

    private Dstu3LibrarySourceProvider librarySourceProvider;
    private Dstu3LibrarySourceProvider getLibrarySourceProvider()
    {
        if (librarySourceProvider == null)
        {
            librarySourceProvider = new Dstu3LibrarySourceProvider(getLibraryResourceProvider());
        }
...
//                buildIncludes(libraries), instance.fhirType(), instance.fhirType(), instance.fhirType(), cql);
        ...
        org.cqframework.cql.elm.execution.Library library = CqlTranslationUtils.translateLibrary(source, getLibraryManager(), getModelManager());
    ...
    )
    {
        ...
        MeasureBundler bundler = new MeasureBundler();

        try
        {
            translator = CqlTranslationUtils.getTranslator(code, getLibraryManager(), getModelManager());
        }
        ...
        org.cqframework.cql.elm.execution.Library library = CqlTranslationUtils.translateLibrary(translator);
        Context context = new Context(library);
        ...
        if (library.getStatements() != null)
        {
            for (org.cqframework.cql.elm.execution.ExpressionDef def : library.getStatements().getDef())
            {
                ...
                try
                {
                    ...
                    Object res =
                            def instanceof org.cqframework.cql.elm.execution.FunctionDef
