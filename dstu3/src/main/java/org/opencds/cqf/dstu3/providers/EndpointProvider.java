...
import org.opencds.cqf.dstu3.utils.ErrorUtils;

import java.util.ArrayList;
import java.util.HashMap;
import java.util.Map;

public class EndpointProvider extends EndpointResourceProvider
{
    private JpaDataProvider provider;
    private IFhirSystemDao systemDao;

    public EndpointProvider(JpaDataProvider provider, IFhirSystemDao systemDao) {
            ...
            @OptionalParam(name="pass") String password)
    {
        ...
        if (endpoint == null)
        {
            return ErrorUtils.createErrorOutcome("Could not find Endpoint/" + theId);
        }
        ...
        if (userName != null || password != null)
        {
            if (userName == null)
            {
                ErrorUtils.createErrorOutcome("Password was provided, but not a user name.");
            }

            else if (password == null)
            {
                ErrorUtils.createErrorOutcome("User name was provided, but not a password.");
            }
        ...
        {
            return ErrorUtils.createErrorOutcome(e.getMessage());
        }
    }
